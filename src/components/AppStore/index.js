...
class AppStore extends Component {
  state = {searchInput: '', currentTabId: tabsList[0].tabId}
  ...
  getUpdatedTabs = searchResults => {
    const {currentTabId} = this.state
    const updatedTabs = searchResults.filter(eachApp =>
      eachApp.category.includes(currentTabId),
    )
    return updatedTabs
  }

  render() {
    const {searchInput, currentTabId} = this.state

    const searchResults = this.getInputSearchResults()
    const updatedTabs = this.getUpdatedTabs(searchResults)

    return (
      <div className="main-container">
        <h1 className="main-heading">App Store</h1>
        <div className="input-element-search-icon-container">
          <input
            type="search"
        ...
        <ul className="ul-tabItem">
          {tabsList.map(eachTab => (
            <TabItem
              eachTabDetails={eachTab}
              key={eachTab.tabId}
              updateCurrentTabsList={this.updateCurrentTabsList}
              isActive={currentTabId === eachTab.tabId}
            />
          ))}
        </ul>
        <ul className="ul-appItem">
          {updatedTabs.map(eachApp => (
